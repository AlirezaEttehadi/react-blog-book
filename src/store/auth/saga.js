...
export function* userLoginSaga(action) {
    ...
    const url = "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDLZweWGjxBltNhwSPoIf4ZZHHO33LP3c4";
    const userData = {
        email: action.email,
...
export function* userSignupSaga(action) {
    yield put(actions.authStart())
    const url = "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDLZweWGjxBltNhwSPoIf4ZZHHO33LP3c4";
    const userData = {
        email: action.email,
        password: action.password,
        returnSecureToken: true
    ...
    try {
        ...
        if(!token) {
            ...
            if(new Date(expirationDate).getTime() > new Date().getTime()) {
                const res = yield call(axios.post, "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyDLZweWGjxBltNhwSPoIf4ZZHHO33LP3c4", {idToken: token})
                console.log("IN HERE");
...
export function* userRestorePasswordSaga(action) {
    ...
    try {
        const res = yield call(
            axios.post,
            "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyDLZweWGjxBltNhwSPoIf4ZZHHO33LP3c4",
            userInfo
        )
        console.log(res);
